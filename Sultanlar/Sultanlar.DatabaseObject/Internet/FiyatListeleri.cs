    ...
    public class FiyatListeleri : ISultanlar, IDisposable
    {
        ...
        public static void GetObjectXml(DataTable dt, short FiyatTipi, string where)
        {
            using (SqlConnection conn = new SqlConnection(General.ConnectionString))
            {
                ...
                if (FiyatTipi == 3 && where == "n11") // 0
                ...
                else if (FiyatTipi == 3 && where == "n11pro") // 0
                    da = new SqlDataAdapter("sp_xml_n11pro", conn);
                    ...
                    da = new SqlDataAdapter("SELECT [SUL_Fiyatlar_18_00_Stok].ITEMREF AS Kod,dbo.IlkHarfBuyuk(dbo.UrunMalzemeKisaltmalar(MALZEME)) AS Baslik,MALZEME AS AltBaslik,CASE WHEN [Web-Malzeme-Aciklama].ACIKLAMA = '' THEN MALZEME ELSE [Web-Malzeme-Aciklama].ACIKLAMA COLLATE DATABASE_DEFAULT END AS Aciklama,(SELECT KATEGORI FROM [Web-Malzeme-ReyonKategori] WHERE REF = [Web-Malzeme-Aciklama].KATEGORIREF) AS KategoriAdi1,/*CONVERT(decimal(18,2),([NET+KDV] / ((100 + KDV) / 100)) * 1.62) AS Fiyat,*/KDV AS Kdv,(SELECT MARKA FROM [Web-Malzeme-Marka] WHERE REF = [Web-Malzeme-Aciklama].MARKAREF) AS KategoriAdi,(SELECT MARKA FROM [Web-Malzeme-Marka] WHERE REF = [Web-Malzeme-Aciklama].MARKAREF) AS Marka,ISNULL(STOKE,0)+STOK AS Stoklar,CONVERT(decimal(18,2),([NET+KDV] / ((100 + KDV) / 100)) * 1) AS Fiyat,'https://www.sultanlar.com.tr/musteri/resim-' + IsNull(CONVERT(VARCHAR(MAX),max(intResimID)),'0') + '.jpg' AS Resim FROM KurumsalWebSAP.dbo.[SUL_Fiyatlar_18_00_Stok] INNER JOIN [Web-Malzeme-Aciklama] ON KurumsalWebSAP.dbo.[SUL_Fiyatlar_18_00_Stok].ITEMREF = [Web-Malzeme-Aciklama].ITEMREF INNER JOIN tblINTERNET_UrunResim ON tblINTERNET_UrunResim.ITEMREF = [SUL_Fiyatlar_18_00_Stok].ITEMREF WHERE ([ANA GRP] = 'AHT' OR [ANA GRP] = 'YEG') AND (SELECT count(REF) FROM [Web-Malzeme-Aciklama] WHERE ITEMREF = [SUL_Fiyatlar_18_00_Stok].ITEMREF AND [KATEGORIREF] != 0 AND [MARKAREF] != 0 AND [CINSIYETREF] != 0) > 0 GROUP BY [SUL_Fiyatlar_18_00_Stok].ITEMREF,MALZEME,[Web-Malzeme-Aciklama].ACIKLAMA,[Web-Malzeme-Aciklama].KATEGORIREF,[Web-Malzeme-Aciklama].MARKAREF,STOKE,STOK,[NET+KDV],KDV ORDER BY MALZEME", conn);
                else if (FiyatTipi == 7) // 0
                    da = new SqlDataAdapter("SELECT [ANA GRP],[ALT GRP],[GRUP],[REYON],KurumsalWebSAP.dbo.[SUL_Fiyatlar_07_00_Stok].[ITEMREF],[KOD],[MALZEME],[BARKOD],[BIRIM],[KOLI],[KDV],[KMP],[F ACIK],[ FIY],[ISK1],[ISK2],[ISK3],[ISK4],[ISK5],[ISK6],[ISK7],[ISK8],[ISK9],[ISK10],[NET+KDV],30 AS [VD],STOK AS STOK,'https://www.sultanlar.com.tr/musteri/resim-' + IsNull(CONVERT(VARCHAR(MAX),max(intResimID)),'0') + '.jpg' AS Resim FROM KurumsalWebSAP.dbo.[SUL_Fiyatlar_07_00_Stok] LEFT OUTER JOIN tblINTERNET_UrunResim ON KurumsalWebSAP.dbo.[SUL_Fiyatlar_07_00_Stok].ITEMREF = tblINTERNET_UrunResim.ITEMREF WHERE [MALZEME] NOT LIKE '%&&%' AND [MALZEME] NOT LIKE 'CHEP PALET%' AND [MALZEME] NOT LIKE 'AĞAÇ PALET%' AND [MALZEME] NOT LIKE 'ZIPBAG%' AND [MALZEME] NOT LIKE 'ALTIN%' GROUP BY [ANA GRP],[ALT GRP],[GRUP],[REYON],KurumsalWebSAP.dbo.[SUL_Fiyatlar_07_00_Stok].[ITEMREF],[KOD],[MALZEME],[BARKOD],[BIRIM],[KOLI],[KDV],[KMP],[F ACIK],[ FIY],[ISK1],[ISK2],[ISK3],[ISK4],[ISK5],[ISK6],[ISK7],[ISK8],[ISK9],[ISK10],[NET+KDV],[VD],[STOK] ORDER BY MALZEME", conn);
                else if (FiyatTipi == 8)
