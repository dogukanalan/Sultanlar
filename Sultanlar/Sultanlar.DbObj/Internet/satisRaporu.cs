...
namespace Sultanlar.DbObj.Internet
{
    ...
    public class satisDashboard : DbObj
    {
        ...
        public List<SatisHedefVgb> GetSatisHedefVgb(int YIL, int AY)
        {
            ...
            if (dic != null)
                for (int i = 0; i < dic.Count; i++)
                    donendeger.Add(new SatisHedefVgb(dic[i][0].ToString(), Convert.ToInt32(dic[i][1]), Convert.ToInt32(dic[i][2]), Convert.ToInt32(dic[i][3]), Convert.ToInt64(dic[i][4]), Convert.ToDouble(dic[i][5]), Convert.ToDouble(dic[i][6])));
