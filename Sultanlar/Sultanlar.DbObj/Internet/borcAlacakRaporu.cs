...
namespace Sultanlar.DbObj.Internet
{
    public class borcAlacakRaporu : DbObj
    {
        ...
        public double VGBVADE { get; set; }
        public double RISKTOP { get; set; }
        public double RISKLMT { get; set; }

        public borcAlacakRaporu() { }
        private borcAlacakRaporu(int GMREF, double BORC, double ALACAK, double BAKIYE, DateTime ORTVADE, double VGB, double VGBVADE, double RISKTOP, double RISKLMT)
        {
            this.GMREF = GMREF;
            this.BORC = BORC;
            this.ALACAK = ALACAK;
        ...
        public List<borcAlacakRaporu> GetObjects(int SLSREF)
        {
            ...
            if (dic != null)
                for (int i = 0; i < dic.Count; i++)
                    donendeger.Add(new borcAlacakRaporu(ConvertToInt32(dic[i][0]), ConvertToDouble(dic[i][1]), ConvertToDouble(dic[i][2]), ConvertToDouble(dic[i][3]), ConvertToDateTime(dic[i][4]), ConvertToDouble(dic[i][5]), ConvertToDouble(dic[i][6]), ConvertToDouble(dic[i][7]), ConvertToDouble(dic[i][8])));
