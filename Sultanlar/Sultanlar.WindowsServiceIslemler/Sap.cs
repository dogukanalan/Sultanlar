...
namespace Sultanlar.WindowsServiceIslemler
{
    public class Sap
    {
        static int tekrarcekilecek = 5;
        static readonly NetworkCredential nc1 = new NetworkCredential("MISTIF", "Ankara1923*+B");

        #region KampanyalarC
        public static void KampanyalarC()
        {
        ...
        #region FiyatlarCsap
        public static void FiyatlarCsap()
        {
            SqlConnection conn = new SqlConnection(General.ConnectionString);
            ...
            if (oncekifiyatsayisi - 5000 < listMaterialPrices.Length)
            {
                ...
                SqlCommand cmd6 = new SqlCommand("sp_SAP_YENI_FIYAT_TIP", conn);
        ...
        #region MalzemelerC
        public static void MalzemelerC(bool malzeme, bool olcubirim)
        {
            SqlConnection conn = new SqlConnection(General.ConnectionString);
            ...
            SqlCommand cmd13 = new SqlCommand("SELECT count(*) FROM [Web-Malzeme-Full]", conn);
            cmd13.CommandTimeout = 1000;
            conn.Open();
            int oncekimalzemesayisi = Convert.ToInt32(cmd13.ExecuteScalar());
            conn.Close();
            ...
            if (oncekimalzemesayisi - 1000 > listMaterials.Length)
            {
                cmdLog.Parameters.AddWithValue("@strLog", listMaterials.Length.ToString() + " Satır" + " (Yazma yapılmadı)");
                conn.Open(); cmdLog.Parameters.AddWithValue("@dtBitis", DateTime.Now); cmdLog.ExecuteNonQuery(); conn.Close();
                return;
            }
            ...
            if (malzeme)
            {
                ...
                SqlCommand cmd14 = new SqlCommand("DROP TABLE [Web-Malzeme-Full-Onceki] SELECT * INTO [Web-Malzeme-Full-Onceki] FROM [dbo].[Web-Malzeme-Full] DROP TABLE [Web-Malzeme-Onceki] SELECT * INTO [Web-Malzeme-Onceki] FROM [dbo].[Web-Malzeme]", conn);
                cmd14.CommandTimeout = 600;
                conn.Open();
...
", conn);
        ...
        #region PersonellerC
        public static void PersonellerC()
        {
            SqlConnection conn = new SqlConnection(General.ConnectionString);
            ...
            SqlCommand cmd2 = new SqlCommand("UPDATE [Web-SatisTemsilcileri] SET [SAT KOD1] = (SELECT TOP 1 [SAT KOD1] FROM [Web-Musteri] WHERE SLSREF = [Web-SatisTemsilcileri].SLSMANREF ORDER BY [SAT KOD1] DESC)", conn);
            cmd2.CommandTimeout = 1000;
            conn.Open();
            cmd2.ExecuteNonQuery();
            conn.Close();
        ...
        #region MusterilerC
        public static void MusterilerC()
        {
            SqlConnection conn = new SqlConnection(General.ConnectionString);
        ...
        #region getekstre
        public static void GetEkstre(DateTime Baslangic)
        {
        ...
        public static void GetEkstre2()
        {
            DateTime Baslangic = Convert.ToDateTime("01.01.2014");
            ...
            DateTime sontarih = MaxTarihGetir(conn, "SAP_Ekstre").AddDays(-2);
            string sonTarih = sontarih.ToString("yyyyMMdd");
            ...
            for (int j = 0; j < yillar.Count; j++)
            {
                ...
                try
                {
                    yirmiuc = ekstre.ZwebSelectEkstre(sonTarih, "", yillar[j].ToString(), "", out yirmiuc2);
                    SAPs.LogYaz("ekstre deneme", true, "sap yanıt verdi, " + yillar[j].ToString() + " yılı verisi " + sonTarih + " için alındı, " + (yirmiuc.Length + yirmiuc2.Length).ToString() + " satır", DateTime.Now, DateTime.Now);
                    ayniyiltekrar = 0;
                }
                catch (Exception ex)
                {
                    SAPs.LogYaz("ekstre deneme", true, "sap hata döndürdü " + yillar[j].ToString() + " - " + sonTarih + ":" + ex.Message, DateTime.Now, DateTime.Now);
                    buyilhatayok = false;
                    if (ayniyiltekrar < 3)
                    {
                        j--;
                ...
                if (buyilhatayok)
                {
                    ...
                    for (int i = 0; i < yirmiuc.Length; i++)
                    {
                        ...
                        {
                            hata = ex.Message;
                            SAPs.LogYaz("ekstre deneme", true, "musteri ekstresi bellege alinirken bir satirda hata olustu: " + ex.Message, DateTime.Now, DateTime.Now);
                        }
                    }
                    #endregion
                    ...
                    for (int i = 0; i < yirmiuc2.Length; i++)
                    {
                        ...
                        {
                            hata2 = ex.Message;
                            SAPs.LogYaz("ekstre deneme", true, "tedarikci ekstresi bellege alinirken bir satirda hata olustu: " + ex.Message, DateTime.Now, DateTime.Now);
                        }
                    }
                    #endregion
                    ...
                    if (hata == string.Empty && hata2 == string.Empty)
                    {
                        ...
                        {
                            hata2 = ex.Message;
                            SAPs.LogYaz("ekstre deneme", true, "sap_ekstre_deneme tablosu doldurulurken hata olustu: " + ex.Message, DateTime.Now, DateTime.Now);
                        }
                    }
                    #endregion

                    SAPs.LogYaz("ekstre deneme", true, yillar[j].ToString() + " yılı " + sonTarih + " sonraki değişiklik olan satırlar yazıldı, " + dt.Rows.Count.ToString() + " satır", bas, DateTime.Now);
                }
            }



            MaxTarihYaz(conn, "SAP_Ekstre", baslangic);
            SAPs.LogYaz("ekstre deneme", hata != string.Empty && hata2 != string.Empty ? false : true, (hata + " " + hata2).Trim(), baslangic, DateTime.Now);
        }
        #endregion
        ...
        public static void GetSAP(bool vbfa, bool siparis, bool teslimat, bool nakilmalfatura, bool nakilsiparis, bool malcikis, bool fatura, bool kolietiket)
        {
                        ...
                        for (int j = 0; j < oniki.Length; j++)
                        {
                            try
                            {
                                ...
                                SqlCommand cmd = new SqlCommand(
                                    ...
                                    "INSERT INTO [SAP_SIPARIS_DETAY] (KAYIT_TARIHI,[Vbeln],[Posnr],[Matnr],[Arktx],[Pltyp],[Ptext],[Zterm],[Netdt],[Matkl],[Wgbez],[Spart],[Vtext2],[Ean11],[Kwmeng],[Vrkme],[Vrkme Text],[Kwmeng2],[Vrkme2],[Vrkme2Text],[Ean112],[Yuz1],[Yuz2],[Yuz3],[Yuz4],[Yuz5],[Yuz6],[Yuz7],[Yuz8],[Yuz9],[Yuz10],[Brtpr],[Netpr],[Netwr],[Kzwi1],[Kdvoran],[Mwsbp],[Waerk],[Brgew],[Ntgew],[Gewei],[Volum],[Voleh],[Ksc1],[Ksc2],[Ksc3],[Ksc4],[Ksc5],[Ksc6],[Ksc7],[Ksc8],[Ksc9],[Ksc10],[Zzpirbz],[Zzpirgt],[Abgru],[Bezei_ab],Pstyv) VALUES (@KAYIT_TARIHI,@Vbeln,@Posnr,@Matnr,@Arktx,@Pltyp,@Ptext,@Zterm,@Netdt,@Matkl,@Wgbez,@Spart,@Vtext2,@Ean11,@Kwmeng,@Vrkme,@VrkmeText,@Kwmeng2,@Vrkme2,@Vrkme2Text,@Ean112,@Yuz1,@Yuz2,@Yuz3,@Yuz4,@Yuz5,@Yuz6,@Yuz7,@Yuz8,@Yuz9,@Yuz10,@Brtpr,@Netpr,@Netwr,@Kzwi1,@Kdvoran,@Mwsbp,@Waerk,@Brgew,@Ntgew,@Gewei,@Volum,@Voleh,@Ksc1,@Ksc2,@Ksc3,@Ksc4,@Ksc5,@Ksc6,@Ksc7,@Ksc8,@Ksc9,@Ksc10,@Zzpirbz,@Zzpirgt,@Abgru,@Bezei_ab,@Pstyv)", conn);
                                #region parameters
                                cmd.Parameters.AddWithValue("@KAYIT_TARIHI", DateTime.Now);
                                ...
                                cmd.Parameters.AddWithValue("@Pstyv", oniki[j].Pstyv);
                                #endregion
                                cmd.ExecuteNonQuery();
                            }
                            catch (Exception ex)
                        ...
                        for (int j = 0; j < alti.Length; j++)
                        {
                            try
                            {
                                ...
                                SqlCommand cmd = new SqlCommand(
                                    ...
                                    "INSERT INTO [SAP_TESLIMAT_BASLIK] (KAYIT_TARIHI,[Lfart],[Vtext],[Vbeln],[Ernam],[Erzet],[Erdat],[Sevkalani],[Sevkyeri],[Teslimat Aciklama],[Volum],[Btgew],[Voleh],[Gewei],[Lifex],Vkorg,Doctp) VALUES (@KAYIT_TARIHI,@Lfart,@Vtext,@Vbeln,@Ernam,@Erzet,@Erdat,@Sevkalani,@Sevkyeri,@TeslimatAciklama,@Volum,@Btgew,@Voleh,@Gewei,@Lifex,@Vkorg,@Doctp)", conn);
                                #region parameters
                                cmd.Parameters.AddWithValue("@KAYIT_TARIHI", DateTime.Now);
                                ...
                                cmd.Parameters.AddWithValue("@Doctp", alti[j].Doctp);
                                #endregion
                                cmd.ExecuteNonQuery();
                            }
                            catch (Exception ex)
                    ...
                    if (onuc != null)
                    {
                        for (int j = 0; j < onuc.Length; j++)
                        {
                            try
                            {
                                ...
                                cmd.Parameters.AddWithValue("@Pstyv", onuc[j].Pstyv);
                                #endregion
                                cmd.ExecuteNonQuery();
                            }
                            catch (Exception ex)
            ...
            Sap.MalzemelerC(false, true); // ölçü birim tablo yenilemesi satisupdate ile çakışmaması için
        ...
        public static void GetSAPgece(bool satisupdate)
        {
            SqlConnection conn = new SqlConnection(General.ConnectionString);
        ...
        static void TarihSaat(DateTime maxtarih, DateTime cekilecektarih, out string tarih, out string saat)
        {
        ...
        static DateTime MaxTarihGetir(SqlConnection conn, string alan)
        {
        ...
        static void MaxTarihYaz(SqlConnection conn, string alan, DateTime bitistarih)
        {
        ...
        static void SatisUpdate(SqlConnection conn)
        {
            DateTime baslangic = DateTime.Now;
            string hata = string.Empty;
        ...
        static int GetFiyatTipiFromGMREF(int GMREF)
        {
            int donendeger = 0;
